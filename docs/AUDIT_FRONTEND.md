...
| `src/services/apiClient.ts:37-43` | Refresh token masih TODO. Tidak ada retry request, token rotation, atau forced logout saat refresh gagal. | Sedang | Selesaikan auth interceptor setelah kontrak backend ditetapkan. |
...
5. Token/session API tidak disimpan atau dipasang secara konsisten.
